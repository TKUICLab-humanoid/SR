...
//				tool->Delay(1500);
		...
		case CBD:	//4.20
			...
			tool->Delay(250);
			/**********************************************************************************************************************/	
			for(i=HeadpositionY;i<1400;i+=10)
			{
				ros_com->sendHeadMotor(HeadMotorID::VerticalID,i,200);
				tool->Delay(50);
				if(liftandcarryinfo->WhichStair == Stair_0)
				{
					if( (strategy_info->label_model[ ImageWidth * 0 + liftandcarryinfo->LeftFoot.XMin ] == SecColor )||(strategy_info->label_model[ ImageWidth * 0 + liftandcarryinfo->RightFoot.XMax ] == SecColor))//5.5
					break;
				}
				else if(liftandcarryinfo->WhichStair == Stair_1)
				{
					if( (strategy_info->label_model[ ImageWidth * 0 + liftandcarryinfo->LeftFoot.XMin ] ==  TopColor )||(strategy_info->label_model[ ImageWidth * 0 + liftandcarryinfo->RightFoot.XMax ] ==  TopColor))
					break;
				}
				else if(liftandcarryinfo->WhichStair == Stair_3)
				{
					if( (strategy_info->label_model[ ImageWidth * 0 + liftandcarryinfo->LeftFoot.XMin ] == TrdColor)||(strategy_info->label_model[ ImageWidth * 0 + liftandcarryinfo->RightFoot.XMax ] == TrdColor))
					break;
				}
				else if(liftandcarryinfo->WhichStair == Stair_4)
				{
					if( (strategy_info->label_model[ ImageWidth * 0 + liftandcarryinfo->LeftFoot.XMin ] == FileColor)||(strategy_info->label_model[ ImageWidth * 0 + liftandcarryinfo->RightFoot.XMax ] == FileColor))
					break;
				}
			}
			ros::spinOnce();
			...
			if(wood_C =true)//中間可以走
			{
				liftandcarryinfo->BodyState =  BigFront;
			}
			else if(wood_C = false)//中間不能走
			{	//優先右轉
				if(wood_L = true)
					liftandcarryinfo->BodyState = BigLeftRotation;
				else if(wood_R = true)
			...
			tool->Delay(50);
			ROS_INFO("Wood_R:%d,Wood_C:%d,Wood_R:%d",edge_distance[5],edge_distance[15],edge_distance[25]);//5.5
			if(liftandcarryinfo->BodyState = BigLeftRotation)
			{
				ros_com->sendBodyAuto(BigLeftRotationX,BigLeftRotationY,0,BigLeftRotationTha,WalkingMode::ContinuousStep,SensorMode(BigLeftRotationimu));
				ROS_INFO("---------------------------");
			...
			else if(liftandcarryinfo->BodyState =  BigFront)//5.5
			{
					
				if(liftandcarryinfo->WhichStair == Stair_0)//在第0層
					ros_com->sendBodyAuto(st0_SprX,st0_SprY,0,st0_SprTha+Theta_offset,WalkingMode::ContinuousStep,SensorMode(st0_Sprimu));//執行一般步態
				else
					ros_com->sendBodyAuto(SprX,SprY,0,SprTha,WalkingMode::ContinuousStep,SensorMode(Sprimu));//執行特殊步態
				ROS_INFO("---------------------------");
				ROS_INFO("Spr!!");
				ROS_INFO("---------------------------");
				tool->Delay(2000);
