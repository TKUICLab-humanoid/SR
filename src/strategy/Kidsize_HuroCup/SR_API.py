...
#距離值
        self.TD = 15                   # 左右腳誤差，用於邊界調整
        self.Du = 10                  #上板前停下的可接受誤差
        self.Iu = 15                    #上板緊急停下數值
        self.Id = 10                    #下板緊急停下數值
        self.Su = 40                  #上板停下數值
        self.Sd = 20                  #下板停下數值
...
#sendContinuousValue
        self.CFx = 0                                              # 最終X輸入值，初始化為0
        self.CGx = 200                                        # 每次漸緩的值

        self.C1x = 600                                        # 起步直走x值
        self.C2x = -300                                        # 微調後退值
        self.C3x = -200                                       # 原地微調x值
        self.C4x = self.C1x - self.CGx            #漸緩值1
        self.C5x = self.C4x - self.CGx            #漸緩值2

        self.C1z = -4                    # 一般右轉z值 / C3z + 2
        self.C2z = 4                     # 一般左轉z值 / C3z + 10
        self.C3z = -6                    # 直走z值

        self.C1y = -800                # 右踏y值 / C3y - 240
        self.C2y = 100                  # 左踏y值 / C3y + 660
        self.C3y = -560                 # 原地微調y值/直走/不踏步y值

#Delay時間
        self.Delay = 2                   # 用於停下機器人/調整磁區
        self.DelayUP = 4             # 機器人上板

#磁區呼叫
        self.Sector = 2      # 上/下板磁區初始化
        # self.Sector = 1      #下板磁區
        # self.Sector = 2      #上紅板磁區
        # self.Sector = 3      # /上黃藍板磁區
        
        # self.Color == 32

#降速處理
    def speed_revision(self):
        if self.lldistance >= self.L1 or self.rrdistance >= self.L1:
            self.CFx = self.C1x
        elif self.L2 < self.lldistance < self.L1 or self.L2 < self.rrdistance < self.L1:
            self.CFx = self.C4x
        elif self.Su < self.lldistance <= self.L2 or self.Su < self.rrdistance <= self.L2:
            self.CFx = self.C5x
        elif self.lldistance <= self.Su or self.rrdistance <= self.rrdistance:
            self.CFx = self.C3x

#磁區設定
    def Sector_Divison(self):
        if self.Times == 1:
            self.SectorUP  = 2
        elif self.Times == 2 or self.Times == 3:
            self.SectorUP  = 3
        elif self.Times >= 4:
            self.SectorUP  = 1
...
#A校正機器人對板子的方位（上板）
    def revision(self):    
        if self.Walk == True and self.Progress == True:
            print(' 此時X值:',self.CFx, end='\n\n')                     #確認邊界的值
            if self.lldistance < self.lrdistance and self.rrdistance < self.rldistance:             #凹面判斷
                print (' 凹面判斷中...',end ='\n\n')
                if self.rldistance < self.lrdistance:
                    print(' 右腳在前，向右轉',end ='\n\n')
                    send.sendContinuousValue(self.CFx,self.C3y,0,self.C1z,0)  #self.C1x=200,self.C1z=左轉值
                    if self.lrdistance < self.Su and self.rldistance < self.Su:
                        print(' 機器人到達，準備微調',end = '\n\n')
                        self.Walk = False
                elif self.rldistance > self.lrdistance:
                    print(' 左腳在前，向左轉',end ='\n\n')
                    send.sendContinuousValue(self.CFx,self.C3y,0,self.C2z,0)
                    if self.lrdistance < self.Su and self.rldistance < self.Su:
                        print(' 機器人到達，準備微調',end = '\n\n')
                        self.Walk = False
                elif self.rldistance == self.lrdistance:
                    print(' 直走，不調整',end = '\n\n')
                    send.sendContinuousValue(self.CFx,self.C3y,0,self.C3z,0)   #C3z=直走值
                ...
                if self.lrdistance < self.Su and self.rldistance < self.Su:
                    print(' 機器人到達，準備微調',end = '\n\n')
                    self.Walk = False
                elif self.b1distance != 999 and self.b2distance ==999 or self.lldistance - self.lrdistance <= self.rldistance - self.rrdistance:
                ...
                if self.b2distance > self.Bd:
                    ...
                    send.sendContinuousValue(self.CFx,self.C3y,0,self.C1z,0)
                else:
                    if self.lldistance - self.rrdistance < self.TD:
                        print(' 右踏，校正板子位置',end = '\n\n')
                        send.sendContinuousValue(self.C3x,self.C1y,0,self.C3z,0)
                ...
                if self.b1distance > self.Bd:
                    ...
                    if self.rrdistance - self.lldistance < self.TD:
                        print(' 左踏，校正板子位置',end = '\n\n')
                        send.sendContinuousValue(self.C3x,self.C2y,0,self.C3z,0)
                    elif self.lldistance < self.rrdistance:
                        print(' 左腳在前（邊），左轉微調',end = '\n\n')
                ...
                print(' 直走，不調整',end = '\n\n ')
                send.sendContinuousValue(self.CFx,self.C3y,0,self.C3z,0)
...
#C到板子前停下，原地校正位置
    def revision_stop(self):
        if self.Walk == False and self.Progress == True:
            if self.lldistance < self.SL or self.lrdistance < self.SL or self.rldistance < self.SL or self.rrdistance < self.SL:
                print('過於靠板，微調後退中...')
                send.sendContinuousValue(self.C2x,self.C3y,0,self.C3z,0)
                ...
                if self.rrdistance - self.lldistance > self.Du:   #self.Du=15
                    ...
                    if self.Times <=3:
                        ...
                        time.sleep(self.Delay)
                        send.sendBodySector(self.Sector)
                        time.sleep(self.Delay)
                        send.sendBodyAuto(self.B2x,0,0,0,2,0)
                        time.sleep(self.DelayUP)
                        send.sendBodySector(29)
                        time.sleep(self.Delay)
                        self.Progress = False
                        self.Walk = False
