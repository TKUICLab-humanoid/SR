...
class deep_calculate:
    ...
    def convert(self, imgmsg):
        ...
        img  = img[round(ROI[0][0]):round(ROI[0][1]),round(ROI[1][0]):round(ROI[1][1])]
    ...
    def calc_slope(self, x0, y0, x1, y1):
        if x0 == x1:
            return float("inf")     #沒有斜率,給無限防止
        return (y1 - y0) / (x1 - x0)

    def edge(self, img, color):
        #遮罩
        for i in range(0, LENTH_ROI, 1):
            for j in range(0, WIDTH_ROI, 1):
        ...
        if lines is not None:
            ...
            x1, y1, x2, y2 = lines[min_y_idx]
            self.now_line_coordinate[0] = x1+115/MULT
            self.now_line_coordinate[2] = x2+115/MULT
            self.now_line_coordinate[1] = y1+120/MULT
            self.now_line_coordinate[3] = y2+120/MULT
        else:
