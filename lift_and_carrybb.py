...
ROUTE_PLAN_FORWARD         = [4000, 800, -500, 0, -2000, -1000]
ROUTE_PLAN_TRANSLATION     = [1500, 2500, 500, 1000, 2000, -1000]
...
BACK_MIN                   = -1000                  #小退後
BACK_NORMAL                = -2000                 #退後
FORWARD_MIN                = 2500                  #小前進
FORWARD_NORMAL             = 3500                  #前進
FORWARD_BIG                = 4000                  #大前進
FORWARD_SUPER              = 3500                  #超大前進
##平移值
TRANSLATION_MIN            = 1800                   #小平移
TRANSLATION_NORMAL         = 2000                  #平移
TRANSLATION_BIG            = 2500                  #大平移
##旋轉值
THETA_MIN                  = 2                     #小旋轉
THETA_NORMAL               = 4                     #旋轉
THETA_BIG                  = 7                     #大旋轉
...
#LC主策略
    ...
    #步態函數,用於切換countiue 或 LC 步態
        ...
        if motion == 'ready_to_lc':
            ...
            if self.layer < 7:
                ...
                if self.layer != 4:
                    if self.layer != 6 and self.layer != 3:
                        self.next_board = ObjectInfo(BOARD_COLOR[self.layer+1],'Board') #設定下一個尋找的板子
    ...
    def edge_judge(self):
    #邊緣判斷,回傳機器人走路速度+ 20+ 20與走路模式
        #if (max(self.distance[0],self.distance[1],self.distance[2])) < GO_UP_DISTANCE +10 and \
        if (min(self.distance[0],self.distance[1],self.distance[2])) < GO_UP_DISTANCE +2 and \
           (min(self.distance[3],self.distance[4],self.distance[5])) < GO_UP_DISTANCE +5 and \
           ...
           #上板
            ...
            return 'ready_to_lc'
             ...
             (max(self.distance[0],self.distance[1],self.distance[2])) < GO_UP_DISTANCE + 5 and \
             (min(self.distance[3],self.distance[4],self.distance[5])) < GO_UP_DISTANCE + 5 and \
             (max(self.distance[3],self.distance[4],self.distance[5])) < GO_UP_DISTANCE + 10 and \
           self.layer >= 4:
            self.state = "下板"
            ...
            #即將踩板
                ...
                # else:
                if self.layer < 4:
                    if max(self.distance[0],self.distance[1],self.distance[2])>60:
                        self.forward = BACK_MIN+FORWARD_CORRECTION 
                        self.translation = RIGHT_THETA * TRANSLATION_BIG + TRANSLATION_CORRECTION
                        self.theta   =  LEFT_THETA * THETA_MIN + THETA_CORRECTION 
                        self.state   = "!!!右平移!!!"
                    elif max(self.distance[3],self.distance[4],self.distance[5])>60:
                        self.forward = BACK_MIN+FORWARD_CORRECTION 
                        self.translation = LEFT_THETA * TRANSLATION_BIG + TRANSLATION_CORRECTION
                        self.theta   =  RIGHT_THETA * THETA_MIN + THETA_CORRECTION 
                        self.state   = "!!!左平移!!!"
                    else:
                        self.forward = BACK_MIN+FORWARD_CORRECTION
                        self.theta_change()
