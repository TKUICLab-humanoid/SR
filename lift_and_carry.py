...
HEAD_VERTICAL              = 1358                  #頭垂直 #down 2750
##判斷值
...
#LC主策略
           ...
           self.layer >= 4:
            ...
            #即將踩板
                if self.layer == 4:
                 ...
                 (self.distance[2] < FIRST_FORWORD_CHANGE_LINE) and (max(self.distance[3],self.distance[4],self.distance[5])>100):
            #左平移
                if self.layer != 1:
                    self.forward     = FORWARD_CORRECTION
                else:
                ...
                self.state ="左平移"
                 ...
                 (self.distance[5] < FIRST_FORWORD_CHANGE_LINE) and (max(self.distance[0],self.distance[1],self.distance[2])>100):
            #右平移
                if self.layer != 1:
                    self.forward     = FORWARD_CORRECTION
                else:
    ...
    #上板或下板後影像上無下一層板
        if self.now_board.center.x > 160:
            self.theta = RIGHT_THETA * THETA_BIG + THETA_CORRECTION
        else:
            self.theta = LEFT_THETA * THETA_BIG + THETA_CORRECTION
        if self.layer < 4:
