...
BASE_CHANGE                = 100                   
#上下板前進量
LCUP1                      = 19000                 #上板 Y_swing = 7,Period_T = 840,OSC_LockRange = 0.4,BASE_Default_Z = 8,BASE_LIFT_Z = 3.2
LCUP2                      = 19000
LCUP3                      = 19000
...
ROUTE_PLAN_FORWARD         = [ 0,     2000,    300,      800,    200,     500  ]
ROUTE_PLAN_TRANSLATION     = [ 1200,  -800,   -1000,     1000,   400,     800  ]   #pos = left, neg = right
ROUTE_PLAN_THETA           = [   -2,    3,      6,       4,      -6,       -3  ]   #pos = left, neg = right
ROUTE_PLAN_TIME            = [    0,    8,      5,       7,      6,        0   ]
#---微調站姿開關---#
...
#LC主策略
    ...
    def main(self,strategy):
        ...
        if self.layer <4:
            send.sendHeadMotor(2,self.head_Vertical+5,100)#垂直 1340
        else:
            send.sendHeadMotor(2,self.head_Vertical-20,100)#垂直 -數值 更靠近基準線 ＋數值遠離基準線
        ...
        #開啟LC策略
            if self.layer < 7:
                if self.walkinggait_stop and self.first_in:
                    ...
                    send.execute = False
                    if STAND_CORRECT_LC:
                        send.sendBodySector(299)             #LC基礎站姿調整磁區
                        while not send.execute:
        ...
        if motion == 'ready_to_lc' or motion == 'continue_to_lc':
            ...
            if self.layer < 4:
                if GND_BOARD_LC and self.layer == 1:
                    ...
                    send.sendBodySector(299)          #上板前站姿調整
                    while not send.execute:
                ...
                if BOARD_GND_LC and self.layer == 6:
                    ...
                    send.sendBodySector(2999)          #下板前站姿調整
                    while not send.execute:
            ...
            rospy.sleep(1)
            if STAND_CORRECT_LC:
                send.sendBodySector(299)              #基礎站姿調整
                while not send.execute:
                    rospy.logdebug("站立姿勢調整\033[K")
                send.execute = False
            rospy.sleep(1)
