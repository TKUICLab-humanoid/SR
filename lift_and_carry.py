...
BASE_CHANGE                = 100                   
#上下板前進量
LCUP1                      = 18000                 #上板 Y_swing = 7,Period_T = 840,OSC_LockRange = 0.4,BASE_Default_Z = 8,BASE_LIFT_Z = 3.2
LCUP2                      = 18000
LCUP3                      = 19000
LCDOWN4                    = 20000                 #下板 Y_swing = 7,Period_T = 840,OSC_LockRange = 0.4,BASE_Default_Z = 8,BASE_LIFT_Z = -1.5
LCDOWN5                    = 20000
LCDOWN6                    = 20000
#上下板後路徑規劃
ROUTE_PLAN_FLAG            = True
ROUTE_PLAN_FORWARD         = [ 0,     1000,   -400,      300,    200,     1000 ]
ROUTE_PLAN_TRANSLATION     = [ 1200,   400,    500,     -600,    500,     800  ]   #pos = left, neg = right
ROUTE_PLAN_THETA           = [   -2,   0,      -7,       7,      -3,        3  ]   #pos = left, neg = right
ROUTE_PLAN_TIME            = [    0,   0,      9,       0,      0,       0   ]
#---微調站姿開關---#
...
#----------#                       右腳           左腳
#                              左 ,  中,  右|  左,  中,   右
FOOT                       = [100 , 123, 144, 174, 194, 215]  #皎皎對板子的位置
HEAD_HORIZONTAL            = 2048                  #頭水平
HEAD_VERTICAL              = 1330                #頭垂直 #down 2750
##判斷值
...
FORWARD_NORMAL             = 1200                  #前進
FORWARD_BIG                = 1700                  #大前進 ////////////////////////////////////////
...
#LC主策略
    ...
    def main(self,strategy):
        ...
        if self.layer <4:
            send.sendHeadMotor(2,self.head_Vertical+10,100)#垂直 1340
        else:
            send.sendHeadMotor(2,self.head_Vertical-10,100)#垂直 -數值 更靠近基準線 ＋數值遠離基準線
        ...
        #開啟LC策略
            if self.layer < 7:
                if self.walkinggait_stop and self.first_in:
                    ...
                    send.execute = False
                    if STAND_CORRECT_LC:
                        send.sendBodySector(20)             #LC基礎站姿調整磁區
                        while not send.execute:
                    ...
                    if self.layer > 3:
                        ...
                        if (self.distance[0] == 0 and self.distance[1] == 0 and self.distance[2] == 0 and self.distance[3] == 0) or \
                           ...
                           (self.distance[4] == 0 and self.distance[5] == 0 and max(self.distance) < 2):
                            rospy.logwarn("！！！！！！！！！！直接下板！！！！！！！！！！\033[K") #判斷直接下板
                    ...
                    send.data_check = False
                    self.find_board()
                    self.walkinggait(motion=self.edge_judge()) #邊緣判斷
        rospy.loginfo('￣￣￣￣￣￣￣￣￣￣￣￣￣￣￣￣￣￣￣￣￣￣\033[K')
    ...
    #獲取板子資訊、距離資訊
        ...
        #-------距離判斷-------#
        for i in range(6):
            self.distance[i],now_edge_point[i] = self.return_real_board(outset=FOOTBOARD_LINE,x=FOOT[i],board=self.now_board.color_parameter) #更新數值
        #-----------------#
        if self.layer != 6 or self.layer != 3:
        #除了上最頂層和下最底層以外,偵測上下板空間
            for i in range(6):
                if now_edge_point[i]>240: #偵測有無上下板空間
                    continue
                else:
        ...
        if motion == 'ready_to_lc' or motion == 'continue_to_lc':
            ...
            if self.layer < 4:
                if GND_BOARD_LC and self.layer == 1:
                    ...
                    send.sendBodySector(103)          #上板前站姿調整
                    while not send.execute:
                    ...
                    rospy.sleep(2)
                    rospy.loginfo("準備上板\033[K")
                    send.sendBodySector(10)          #上板前站姿調整
                    while not send.execute:
                        rospy.logdebug("上板前姿勢\033[K")
                    rospy.sleep(1)
                    send.execute = False
                    ...
                    send.sendBodyAuto(LCUP3,0,0,0,2,0)
                else:
                    send.sendWalkParameter('send',\
                                                walk_mode = 2,\
                                                com_y_shift =-3.5,\
                    ...
                    rospy.sleep(1.5)
                    send.sendBodyAuto(LCUP1,0,0,0,2,0)    #上板步態
            else:
                if BOARD_GND_LC and self.layer == 6:
                    ...
                    send.sendBodySector(103)          #下板前站姿調整
                    while not send.execute:
                        rospy.logdebug("下板前姿勢\033[K")
                    rospy.sleep(1)
                    send.execute = False               #微調站姿延遲
                    send.sendBodyAuto(LCDOWN6,0,0,0,3,0)
                    ...
                    rospy.sleep(1)
                    send.execute = False               #微調站姿延遲
                    send.sendBodyAuto(LCDOWN5,0,0,0,3,0)
                else:
                    send.sendWalkParameter('send',\
                                            walk_mode = 3,\
                                            com_y_shift = -5.5,\
    ...
    #邊緣判斷,回傳機器人走路速度與走路模式
        if ((self.distance[0] < GO_UP_DISTANCE+8) and (self.distance[1] < GO_UP_DISTANCE+6) and\
           ...
           self.state = "上板"
           return 'ready_to_lc'
        elif ((self.distance[0] < GO_UP_DISTANCE+2) and (self.distance[1] < GO_UP_DISTANCE+3) and\
           (self.distance[2] < GO_UP_DISTANCE+3) and (self.distance[3] < GO_UP_DISTANCE+3) and\
           (self.distance[4] < GO_UP_DISTANCE+3)and (self.distance[5] < GO_UP_DISTANCE+2)and self.layer == 6):
           self.state = "下底板"
           return 'ready_to_lc'
        elif ((self.distance[0] < GO_UP_DISTANCE+1) and (self.distance[1] < GO_UP_DISTANCE+3) and\
           (self.distance[2] < GO_UP_DISTANCE+3) and (self.distance[3] < GO_UP_DISTANCE+3) and\
           (self.distance[4] < GO_UP_DISTANCE+3)and (self.distance[5] < GO_UP_DISTANCE+1)and self.layer >=4):
           #上板
           self.state = "下板"
           return 'ready_to_lc'
        else:
    ...
    #旋轉修正
        decide_theta = 0
        a = edge.now_line_coordinate[0]
        b = edge.now_line_coordinate[1]
        c = edge.now_line_coordinate[2]
        d = edge.now_line_coordinate[3]
    ...
    #上板或下板後影像上無下一層板
        ...
        if self.layer != 4:
            if self.now_board.edge_min.x >= 162:
                rospy.loginfo(f"aaaaaaaaaaaaaaaaaaaaaa")
